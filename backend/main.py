...
class AnalysisRequest(BaseModel):
    ...
    query: str
    model_choice: str = "gemini" # Add model_choice, default to gemini
...
def analyze_data(request: AnalysisRequest):
    ...
    if len(valid_cols_for_trend) >= 2:
        ...
        if len(trend_df) > 1:
            X = trend_df[[col1]].values; y = trend_df[[col2]].values
            try:
            ...
            except Exception as e: print(f"Trend analysis failed: {e}")
    ...
    # Pass the model_choice from the request to the brain
    insight = brain.think(
